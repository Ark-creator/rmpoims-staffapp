...
const ViewDetailsModal = ({ visible, onClose, orderData, onUpdateStatusRequest }) => {
    ...
    const pendingOrders = orderGroup.filter(o => o.status === 'pending');
    const completedOrders = orderGroup.filter(o => o.status === 'completed');
    const grandTotal = orderGroup.reduce((sum, order) => sum + (order.quantity * (order.exclusive_deal?.price || 0)), 0);

    const OrderRow = ({ order, isHeader }) => {
        const isInsufficient = !isHeader && (order.available_stock < order.quantity);
            ...
            <View style={[modalStyles.tableRow, isInsufficient && modalStyles.insufficientStockRow]}>
                <View style={{flex: 2}}>
                    <Text style={modalStyles.tableCellHeader} numberOfLines={1}>
                        {isHeader ? 'Generic Name' : order.exclusive_deal?.product?.generic_name}
                    </Text>
                    <Text style={modalStyles.tableCellSub} numberOfLines={1}>
                        {isHeader ? 'Brand Name' : order.exclusive_deal?.product?.brand_name}
                    </Text>
                </View>

                <Text style={modalStyles.tableCell}>
                    {isHeader ? 'Form' : order.exclusive_deal?.product?.form}
                </Text>
                
                <Text style={modalStyles.tableCell}>
                    {isHeader ? 'Available' : order.available_stock}
                </Text>
                <Text style={modalStyles.tableCell}>
                    {isHeader ? 'Qty' : order.quantity}
                </Text>
    ...
    return (
        <Modal visible={visible} transparent={true} animationType="fade" onRequestClose={onClose}>
            <View style={modalStyles.overlay}>
                <View style={modalStyles.modalContainer}>
                    <View style={modalStyles.header}>
                        <Text style={modalStyles.headerTitle}>Change Status for:</Text>
                        <TouchableOpacity onPress={onClose}>
                            <MaterialCommunityIcons name="close-circle" size={30} color="#666" />
                        </TouchableOpacity>
                    </View>
    ...
    const handleBarCodeScanned = ({ data }) => {
        try {
            ...
            if (parsedData.order_id && parsedData.product_name) {
                ...
                throw new Error("QR code is missing required order data."); 
            }
        } catch (error) {
            ...
            {!scannedData ? (
                ...
                >
                    ...
                    <View style={styles.layerBottom}>
                        <Text style={styles.scanPrompt}>Point camera at QR code</Text>
                    </View>
                </CameraView>
            ) : (
                <View style={styles.signatureContainer}>
                    ...
                    <View style={styles.buttonRow}>
                        ...
                        >
                            <MaterialCommunityIcons name="eraser" size={18} color="#E53935" />
                            <Text style={[styles.buttonText, styles.clearButtonText]}>Clear</Text>
                        </TouchableOpacity>
                        <TouchableOpacity 
                            style={[styles.button, styles.submitButton, !signature && styles.disabledButton]} 
...
export default function OrdersScreen() {
    ...
    const handleStatusSubmit = async (status) => {
        ...
        try {
            await api.post(`/mobile/staff/order/${selectedProduct.id}/update-status`, { 
                status: status 
            });
            ...
            const filteredGroup = updatedOrderGroup.filter(o => 
    ...
    const companyNames = ['All', ...new Set(
        Object.values(data.ordersByProvince).flatMap(c => Object.keys(c))
    )];
                                ...
                                {Object.entries(companies)
                                    ...
                                    .map(([co, employees]) => {
                                        const filteredEmployees = Object.entries(employees)
                                            .filter(([emp]) => emp.toLowerCase().includes(filters.search.toLowerCase()));
...
const styles = StyleSheet.create({
    ...
    summaryTitle: { fontSize: 14, color: '#666', marginTop: 4 }, 
    alertText: { color: '#FFF' },
    listContainer: { 
        backgroundColor: '#FFFFFF', 
        borderRadius: 12, 
    ...
    orderEmployee: { fontSize: 16, fontWeight: '600' }, 
    ...
    scannerContainer: { flex: 1, backgroundColor: 'white' }, 
    center: { justifyContent: 'center', alignItems: 'center' }, 
    permissionText: { fontSize: 18, textAlign: 'center', padding: 20 },
    ...
    layerTop: { flex: 1.5, backgroundColor: 'rgba(0,0,0,0.6)' }, 
